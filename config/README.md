...
> Bistro FR1 100 MHz 模板使用 3276 个 active subcarrier。正式 64 PRB 生产仿真当前以
...
已完成 `7 BS x 2583 UE` 全量 SRS direct uplink。Front3D 64 PRB 正式任务因 BS 数、
空间谱、可视化和 full path 组合更重，当前生产模板使用 `shard_size: 5`，并依赖
fallback 在少数超大 shard 上继续二分。多个 shard 会分别写 `results/result_xxx.h5`，
...
`perf_summary*.json` 会汇总 completed stage、失败状态、exception type/message、
硬件采样峰值，以及 HDF5 `hdf5.dataset_write` 事件的 raw bytes、storage bytes、
压缩比和最慢/最大的 dataset。普通 pipeline 目前不伪造 link chunk；
`link_log_interval` 主要留给 chunk-aware benchmark 和未来 PHY link batching。

独立性能工程入口不读取 YAML `debug` 段，而是使用 CLI 参数控制 tracing：
...
uv run python -m sionna_measurement_sim.app.cli benchmark rt \
  ...
  --max-bs 1 --max-ue 2 --num-subcarriers 8
...
`label_file` 指向标准 label `0.1.0` JSON。解析器只使用顶层 `bs_points` 和
`ue_points` 作为全场景默认点集，`groups` 保留为房间/区域等子集元数据；当前不需要也
不支持通过配置选择某个 group。点坐标支持 `position: [x, y, z]` 和显式 `x/y/z` 两种
写法。标准 floorplan 文件名形如 `floorplan_1p60.png`，其中 `1p60` 表示 `z=1.60 m`
截断高度。
...
文件体积通常会略增。正式 64 PRB full 任务当前推荐 `gzip_level: 1`：路径表和 CFR truth
仍有明显压缩收益，但比默认 level 4 少消耗大量 CPU。
...
`"frequency"` 只写 `/iq/link/frequency_clean`，`"both"` 同时写两者。`phy.iq` 下旧的
`save_frequency_clean/save_time_clean` 已移除；`iq_link_library` profile 在未显式提供
...
| `iq.clean_output` | str\|null | null | clean IQ 选择器：`"time"`、`"frequency"`、`"both"` |
...
启用 `phy.iq.enabled=true` 时至少要设置 `iq.clean_output` 或打开一个 observed save flag。
