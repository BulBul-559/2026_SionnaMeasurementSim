...
| `sources` | list[str] | ["truth_cfr", "cfr_est", "rx_grid"] | `truth_cfr` 生成真值信道谱；`cfr_est` 生成估计信道谱；`rx_grid` 生成 NR PUSCH 接收信号谱；`srs_cfr_est` 生成 SRS-like LS 估计谱 |
...
antenna:
  tx_array: { num_rows: 4, num_cols: 4 }  # BS receiver array in uplink
  rx_array: { num_rows: 1, num_cols: 2 }  # UE sounding antennas
phy:
  standard: "nr_srs"
