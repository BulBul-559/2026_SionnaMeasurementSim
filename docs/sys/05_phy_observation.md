...
`output.profile="iq_link_library"` 是同一 IQ 层的 compact clean-link 模式。它当前只适配
NR SRS：SRS 模块仍生成标准化 resource/tx grid 并计算 `rx_grid_clean=H*x`，但在
common impairment/AWGN、receiver、resource LS 和 full-band `cfr_est` 之前直接返回。
pipeline 随后只写 clean `/iq/link/time_clean` 和/或 `/iq/link/frequency_clean` 加必要
metadata，不写 `/waveform`、`/observation`、`/channel`、`/paths`、`/array` 或
`/ranging`。该文件用于后续在线叠加 UE 链路，在线阶段再统一添加噪声、时钟偏移或前端损伤。
clean payload 只通过 `phy.iq.clean_output` 在 `"time"`、`"frequency"`、`"both"`
三种取值之间选择；旧的 `save_frequency_clean/save_time_clean` 配置字段已移除。
