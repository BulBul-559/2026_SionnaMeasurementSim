...
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `profile` | str | `"full"` | 输出 profile：`full`、`rt_lite`、`rt_labels_only`、`iq_link_library` 或 `custom` |
| `products` | list[str]\|null | null | 仅 `profile="custom"` 使用；选择要计算/写入的关键产物 |
...
非合作样本。
`profile="custom"` 使用 full contract 的 product-aware 变体，`output.products` 可选
`derived`、`cfr_truth`、`cir_truth`、`path_samples`、`nlos_path_truth`、`path_full`、
...
| Dataset | 类型 | 说明 |
|---------|------|------|
| `schema_version` | string | Schema 版本号，当前为 `2.2.0` |
...
| `output_profile` | string | 输出 profile：`full`、`rt_lite`、`rt_labels_only`、`iq_link_library` 或 `custom` |
| `output_products` | string[] | 解析后的产品列表；旧 full profile 写完整列表，custom 写用户选择后的规范化列表 |
...
schema `2.2.0` 仍只允许 clean `/iq/link`。
