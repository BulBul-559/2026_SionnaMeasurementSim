...
自动切 root append dataset；对于固定 metadata/frequency 这类 shared dataset 会返回整块；
对于写到 `/bundle/fragments/<fragment_id>/...` 的 sidecar 值会优先返回 sidecar，适合训练
loader 或调试脚本避免手写 HDF5 轴映射。
