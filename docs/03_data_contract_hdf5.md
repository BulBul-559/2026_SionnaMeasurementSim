...
- `/channel/truth/cfr` 方向为 RT trace 方向（BS→UE）。
- 对于 NR PUSCH 上行的 observation，系统内部通过 TDD 互易性应用。
- `/observation/cfr_est` 的方向记录在 `/link` metadata 中。
...
当 standard 为 `"nr_pusch"` 时，还需写入：

```text
num_prb                        int32
subcarrier_spacing_khz         int32
num_layers                     int32
...
```text
nmse_db                        float32 [snapshot, tx, rx]  # Primary — NMSE(H_obs, clean H_true)
nmse_db_total                  float32 [snapshot, tx, rx]  # Diagnostic — NMSE vs impaired+noisy (AWGN isolation)
...
- `nmse_db` 是主评估指标，计算 NMSE(H_obs, clean H_true)，维度始终为 `[snapshot, tx, rx]`。
- `nmse_db_total` 是诊断指标，计算观测值与含噪有损信道之间的 NMSE（AWGN 隔离诊断）。
- 兼容性说明：Doc 13 中曾将 `nmse_db_total` 命名为 `nmse_awgn_db`；当前代码为了向后兼容仍使用 `nmse_db_total`。
