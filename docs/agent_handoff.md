...
当前 HDF5 schema 版本是 `2.3.0`。`output.profile` 只保留三种真实契约：
`full`、`rt_labels_only` 和 `iq_link_library`。`rt_labels_only` 使用独立
...
`full` profile 可选 `output.products` 选择关键产物并动态裁剪 RT/PHY/下游链路；例如
`products: ["cfr_truth"]` 只计算/写 `/channel/truth/cfr` 与必要元数据，跳过 CIR、path samples、
PHY observation、ranging、array spectrum、IQ 和 visualization。常用产品包括
`derived`、`link_labels`、`cfr_truth`、`cfr_obs`、`ranging`、`array`、`iq`、
`multiuser`、`path_samples` 和 `all`，其中 `rtt` 是 `ranging` 的别名。`array`
产品按 `array.spectrum.sources` 动态判断依赖：
...
`output.compression` 支持 `gzip`、`lzf`、`none`、`mixed`，`output.gzip_level` 控制
gzip dataset 的压缩等级。正式 64PRB full 仿真推荐从 `mixed` + `gzip_level: 1`
...
释放 Sionna RT / Dr.Jit GPU allocations，保持动态调度的显存空闲判断有效。retryable
fallback 会在拆分子 shard 前清理失败 exception traceback 引用，避免外层 worker 在启动
fallback 子进程时继续持有失败 shard 的大 GPU allocation；该修复在
`docs/performance/front3d_stall_diagnostics_2026-06-21.md` 记录的同一批异常场景、
固定 GPU 5/6、每卡 4 GiB 显存压力下完成 10 次连续无复现回归。实验性
...
选择只保存时域 clean IQ、只保存频域 clean IQ 或两者都保存。`phy.iq` 下旧的
`save_frequency_clean/save_time_clean` 已移除，配置加载会拒绝这些字段。
...
`shard_size=5` 是当前 Front3D 64 PRB full 任务的生产建议。历史报告里出现的 `20/25`
是旧场景或旧实验记录，不再作为正式 64 PRB full 任务默认值。
...
这不是普通显存 OOM，而是底层 Dr.Jit 单数组 entry 数限制。该旧 baseline 后续用
`shard_size=20` 跑通；当前 Front3D 64 PRB full 正式任务模板进一步收紧为
`shard_size=5`。
