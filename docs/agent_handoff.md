...
当前 HDF5 schema 版本是 `2.2.0`。`output.profile` 支持 `full`、`rt_lite`、
`rt_labels_only`、`iq_link_library` 和 `custom`：`rt_lite` 是保留 full contract 的轻量 preset，会关闭
...
`custom` profile 使用 `output.products` 选择关键产物并动态裁剪 RT/PHY/下游链路；例如
