...
smoke 测试；reader 已提供 `iter_manifest_dataset_batches()`，可沿 resolved UE 轴把
manifest fragment 拼接成训练式 batch，并对连续 bundle fragment 复用打开句柄；新增
`benchmark readback` 可对已有 run/manifest/HDF5 单独复测 batch 读取吞吐。默认生产路径仍是
一个 shard 一个 `results/result_xxx.h5`。
...
体积、dataset write event 数、schema validate 时间和 manifest-aware batch CFR readback 时间，但
小 payload 下 bundle writer 固定成本仍明显，且同进程 RT warm cache 会污染 end-to-end wall
time。下一步需要用 `benchmark readback` 在更大真实 shard 和真实训练 loader 上对比默认 shard 文件、bundle 读取吞吐、
