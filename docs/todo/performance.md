...
| 5 | PERF-005 | write-only benchmark | 从固定 shape/dtype 数组直接测 writer、compression、schema validate 和 manifest 成本。 |
| 6 | PERF-006 | spectrum / visualization 开关矩阵 | 隔离评估 spectrum off/on、visualization off/on 和不同 spectrum source 的增量成本。 |
| 7 | PERF-007 | batch size 自适应 | 记录并自动选择不同 GPU、UE/BS 规模和频域配置下稳定 batch 边界。 |
| 8 | PERF-008 | 多 GPU 调度扩展性 | 测 2/4/6/8 GPU shard 扩展性、尾 shard 调度和 GPU 忙碌场景。 |
...
manifest、write benchmark。

验收标准：至少比较 chunk shape、压缩算法、压缩等级、flush 策略、并行写文件数和 schema validate
开关；输出推荐配置和风险说明；真实 shard 或 write-only benchmark 有可复现实验结果。
...
验收标准：建立 RT-only 矩阵，记录耗时、显存、path_count、LoS/NLoS、CFR 差异和失败边界；
...
验收标准：评估 steering matrix cache、按 link chunk 复用、归一化优化和不同 `link_chunk_size`；
...
### PERF-005: write-only benchmark

目的：从已有内存数组或轻量合成数组直接测写盘，避免 RT/PHY/array 成本掩盖 writer 问题。

涉及模块：benchmark CLI/API、HDF5 writer、schema validation、performance docs。

验收标准：构造固定 shape/dtype 的代表数组，输出 writer 阶段耗时、压缩率、文件大小和 schema validate
成本；能作为 PERF-001 的实验基础。

重点提醒：benchmark 数据不要写进 git；输出到 ignored `outputs/` 下清晰命名。
...
验收标准：比较 spectrum off/on、visualization off/on、`truth_cfr`/`cfr_est`/`rx_grid` 等 source
